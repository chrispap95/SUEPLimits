...
# Step 1: Create datacards for Run 2 and combine them
...
job_count=0
for scan_point in auxiliaries/cards/SUEP_signal_scale0.0001_GluGluToSUEP_mS*nic_Run2_13TeV.txt; do 
    (
...
for file in auxiliaries/workspaces/SUEP_signal_scale0.0001_GluGluToSUEP_mS*nic_Run2_13TeV.root; do
    (
...
for mPhi in $(ls auxiliaries/workspaces/ | grep -oP 'mPhi\K[\d\.]+' | sort -u); do
    for T in $(ls auxiliaries/workspaces/ | grep -oP 'T\K[\d\.]+' | sort -u); do
        for mode in hadronic leptonic; do
            output="higgsCombine_scale0.0001_mPhi${mPhi}_T${T}_${mode}_Run2_13TeV.AsymptoticLimits"
