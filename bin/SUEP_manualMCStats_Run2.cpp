    ...
    for (auto signal : signals) {
        ...
        for (size_t i = 0; i < years.size(); ++i) {
            ...
            for (auto const& unc : manual_MC_stats) {
                string target_bin;
                if (unc.compare(0, 5, "CRQCD") == 0) {
                    target_bin = "CR_QCD_" + era;
                } else if (unc.compare(0, 4, "CRDY") == 0) {
                    target_bin = "CR_DY_" + era;
                } else if (unc.compare(0, 6, "SRLowT") == 0 && signal_region == "SR_low_temp") {
                ...
                bool is_sr_stat = unc.compare(0, 6, "SRLowT") == 0 || unc.compare(0, 7, "SRHighT") == 0;
                if (!is_sr_stat) {
                    cb.cp().backgrounds().bin({target_bin}).era({era}).AddSyst(
                        cb,
                        string("MCStat") + string("$PROCESS") + unc + string("_$ERA"),
                        "shape",
                        ch::syst::SystMap<ch::syst::era>::init({era}, 1)
                    );
                    cb.cp().signals().bin({target_bin}).era({era}).AddSyst(
                        cb,
                        string("MCStatSUEP") + unc + string("_$ERA"),
                        "shape",
                        ch::syst::SystMap<ch::syst::era>::init({era}, 1)
                    );
                    continue;
